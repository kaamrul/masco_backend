...
<div class="content-wrapper">
    ...
    <div class="card shadow-sm col-md-6">
        <div class="card-body py-sm-4">
            <form method="post" action="{{ route('admin.team.update', $teamData->id) }}" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-md-12">
                        <div class="p-sm-3">
                            ...
                            <div class="form-group row @error('name') error @enderror">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" name="name"
                                        value="{{ old('name', $teamData->name) }}" placeholder="{{ __('Name') }}"
                                        required>
                                    @error('name')
                                    ...
                                    <select class="form-control select2" name="parent_id" required>
                                        ...
                                        @foreach($team as $key => $category)
                                            <option value="{{ $category->id }}"
                                                {{ old("parent_id") ?? $teamData->parent_id == $category->id ? "selected" : ""}}>
                                                {{ $category->name }}
                                            </option>
                                            ...
                                            @foreach($category->childrenTeams as $key => $subCategory)
                                                ...
                                                <option class="text-capitalize" value="{{ $subCategory->id }}"
                                                    {{ old("parent_id") ?? $teamData->parent_id == $subCategory->id ? "selected" : ""}}>
                                                    &nbsp;-{{ $subCategory->name }}
                                                </option>
                                                @endif
                                                ...
                                                @foreach($subCategory->childrenTeams as $key => $subSubCat)
                                                    ...
                                                    <option class="text-capitalize" value="{{ $subSubCat->id }}"
                                                        {{ old("parent_id") ?? $teamData->parent_id == $subSubCat->id ? "selected" : ""}}>
                                                        &nbsp;&nbsp;--{{ $subSubCat->name }}
                                                    </option>
                                                    @endif
                                                    ...
                                                    @foreach($subSubCat->childrenTeams as $key => $subSub2Cat)
                                                        ...
                                                        <option class="text-capitalize" value="{{ $subSub2Cat->id }}"
                                                            {{ old("parent_id") ?? $teamData->parent_id == $subSub2Cat->id ? "selected" : ""}}>
                                                            &nbsp;&nbsp;&nbsp;---{{ $subSub2Cat->name }}
                                                        </option>
                                                        @endif
                                                        ...
                                                        @foreach($subSub2Cat->childrenTeams as $key => $subSub3Cat)
                                                            @if ($subSub3Cat->teams)
                                                            <option class="text-capitalize" value="{{ $subSub3Cat->id }}"
                                                                {{ old("parent_id") ?? $teamData->parent_id == $subSub3Cat->id ? "selected" : ""}}>
                                                                &nbsp;&nbsp;&nbsp;----{{ $subSub3Cat->name }}
                                                            </option>
                                                            @endif
                                                            ...
                                                            @foreach($subSub3Cat->childrenTeams as $key => $subSub4Cat)
                                                                @if ($subSub4Cat->teams)
                                                                <option class="text-capitalize" value="{{ $subSub4Cat->id }}"
                                                                    {{ old("parent_id") ?? $teamData->parent_id == $subSub4Cat->id ? "selected" : ""}}>
                                                                    &nbsp;&nbsp;&nbsp;-----{{ $subSub4Cat->name }}
                                                                </option>
                                                                @endif
                                                            @endforeach
                                ...
                                <div class="col-sm-9">
                                    <select class="form-control select2" name="team_leader_id" required>
                                        ...
                                        <option value="{{ $user->id }}"
                                            {{ (old("team_leader_id", $teamData->team_leader_id) == $user->id) ? "selected" : "" }}>
                            ...
                            <div class="form-group row @error('description') error @enderror">
                                ...
                                <div class="col-md-9">
                                    ...
                                    value="{{ old('description', $teamData->description) }}" placeholder="Description"></textarea>
                                    @error('description')
                                    <p class="error-message">{{ $message }}</p>
                                    @enderror
                                </div>
