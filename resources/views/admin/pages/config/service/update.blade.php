...
<div class="content-wrapper">
    ...
    {!! \App\Library\Html::linkBack(route('admin.config.more_settings.service.index')) !!}
        <div class="d-block">
            <h4 class="content-title">{{ strtoupper(__('Update Service')) }}</h4>
        </div>
    ...
    <div class="card shadow-sm">
        <div class="card-body py-sm-4">
            <form method="post" id="service-form"  action="{{ route('admin.config.more_settings.service.update', $service->id) }}" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-md-6">
                        <div class="p-sm-3">
                            ...
                            <div class="form-group row @error('name') error @enderror">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" name="name"
                                        value="{{ old('name', $service->name) }}" placeholder="{{ __('Name') }}"
                                        required>
                                    @error('name')
                                ...
                                <div class="col-sm-9">
                                    <select class="form-control select2" name="service_manager_id" id="service_manager" required>
                                        ...
                                        @foreach($employees as $key => $employee)
                                            <option value="{{ $employee->id }}" {{(old("service_manager_id", $service->service_manager_id) == $employee->id) ? "selected" : ""}}>
                                ...
                                <div class="col-sm-9">
                                    <select class="form-control select2" name="location" id="location" required >
                                        ...
                                        @foreach($locations as $value)
                                            <option class="text-capitalize" value="{{ $value->name }}" {{(old("location", $service->location) == $value->name) ? "selected" : ""}}>
                                                {{ $value->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('location')
                            ...
                            <div class="form-group row @error('reporting_frequency') error @enderror">
                                ...
                                <div class="col-sm-9">
                                    <input type="number" class="form-control" name="reporting_frequency" id="reporting_frequency"
                                        value="{{ old("reporting_frequency", $service->reporting_frequency) }}" min="1" placeholder="{{ __('Reporting Frequency') }}"
                                        required>
                            ...
                            <div class="form-group row @error('start_date') error @enderror">
                                ...
                                <div class="col-sm-9">
                                    <input type="date" name="start_date" max="{{ now()->format('Y-m-d') }}" id="start_date" class="form-control"
                                        value="{{ old('start_date', $service->start_date) }}" required>
                                    @error('start_date')
                                        <p class="error-message">{{ $message }}</p>
                                    @enderror
                                </div>
                            ...
                            <div class="form-group row @error('end_date') error @enderror">
                                ...
                                <div class="col-sm-9">
                                    <input type="date" name="end_date" min="{{ now()->format('Y-m-d') }}" id="end_date" class="form-control"
                                        value="{{old('end_date', $service->end_date)}}" required>
                                    @error('end_date')
                                        <p class="error-message">{{ $message }}</p>
                                    @enderror
                                </div>
                        ...
                        <div class="row">
                            <div class="p-sm-3 col-sm-12">
                                ...
                                <div class="form-group row @error('funder_name') error @enderror">
                                    ...
                                    <div class="col-sm-9">
                                        <input type="text" class="form-control" name="funder_name" id="funder_name"
                                            value="{{ old('funder_name', $service->funder_name) }}" placeholder="{{ __('Funder Name') }}"
                                            required>
                                        @error('funder_name')
                                            <p class="error-message">{{ $message }}</p>
                                        @enderror
                                ...
                                <div class="form-group row @error('funder_details') error @enderror">
                                    ...
                                    <div class="col-sm-9">
                                        <textarea type="text" name="funder_details" class="form-control"
                                            placeholder="{{ __('Write About Asset') }}" rows="4">{{ old('funder_details', $service->funder_details) }}</textarea>
                                ...
                                <div class="form-group row @error('description') error @enderror">
                                    ...
                                    <div class="col-sm-9">
                                        <textarea class="form-control" id="summernote" name="description">{{ old('description', $service->description) }}</textarea>
                                        @error('description')
                                            <p class="error-message">{{ $message }}</p>
                                        @enderror
                                    </div>
...
<script>
    $(document).ready(function () {
        ...
        var employees = <?php echo $service->getEmployee() ?> ;

        var employeeArr = [];
        $.each(employees, function(index, row) {
            employeeArr.push(row.id)
        });

        $('#kaimahi_ids').val(employeeArr).trigger("change");
