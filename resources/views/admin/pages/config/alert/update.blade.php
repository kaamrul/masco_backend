...
<div class="content-wrapper">
    ...
    {!! \App\Library\Html::linkBack(route('admin.config.more_settings.alert.index')) !!}
        <div class="d-block">
            <h4 class="content-title">{{ strtoupper(__('Update Alert')) }}</h4>
        </div>
    </div>

    <div class="card shadow-sm col-md-8">
        <div class="card-body py-sm-4">
            <form method="post" action="{{ route('admin.config.more_settings.alert.update', $alert->id) }}" enctype="multipart/form-data">
                @csrf
                <div class="p-sm-3">
                    ...
                    <div class="form-group row @error('name') error @enderror">
                        ...
                        <div class="col-sm-9">
                            <input type="text" class="form-control" name="name"
                                value="{{ old('name', $alert->name) }}" placeholder="{{ __('Write a Alert Name') }}"
                                required>
                            @error('name')
                        ...
                        <div class="col-sm-9">
                            ...
                            <select class="form-control select2" name="parent_alert" required>
                                ...
                                @foreach($parentAlerts as $key => $parent)
                                    <option class="text-capitalize" value="{{ $parent }}" {{ $parent == (old('parent_alert') ?? $alert->parent_alert) ? "selected" : "" }}>
                                        {{ $parent }} 
                                    </option> 
                                @endforeach
                            </select>
                    ...
                    <div class="form-group row @error('details') error @enderror">
                        ...
                        <textarea type="text" name="details" class="form-control"
                                placeholder="{{ __('Write Details.......') }}" rows="8" required>{{ old('details', $alert->details) }}</textarea>
                            @error('details')
                                <p class="error-message">{{ $message }}</p>
                            @enderror
                        </div>
