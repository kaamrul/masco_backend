...
<div class="content-wrapper">
    ...
    {!! \App\Library\Html::linkBack(route('admin.config.more_settings.discount.index')) !!}
        <div class="d-block">
            <h4 class="content-title">{{ strtoupper(__('New Discount')) }}</h4>
        </div>
    </div>

    <div class="card shadow-sm col-md-8">
        <div class="card-body py-sm-4">
            <form method="post" action="{{ route('admin.config.more_settings.discount.create') }}" enctype="multipart/form-data">
                @csrf
                <div class="p-sm-3">
                    ...
                    <div class="form-group row @error('title') error @enderror">
                        ...
                        <div class="col-sm-9">
                            <input type="text" class="form-control" name="title"
                                value="{{ old('title') }}" placeholder="{{ __('Write a Discount Title') }}"
                                required>
                            @error('title')
                        ...
                        <div class="col-sm-9">
                            <div class="input-group with-icon">
                                <input type="text" name="start_date" id="start_date"
                                    ...
                                    value="{{ old('start_date') }}" placeholder="{{ settings('date_format') }}">
                        ...
                        <div class="col-sm-9">
                            <div class="input-group with-icon">
                                <input type="text" name="end_date" id="end_date"
                                    class="form-control datepicker-min-today"
                                    value="{{ old('end_date') }}" placeholder="{{ settings('date_format') }}">
                                <i class="date-icon fa-solid fa-calendar-days"></i>
                            </div>
                            @error('end_date')
                                <p class="error-message">{{ $message }}</p>
